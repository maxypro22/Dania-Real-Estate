...
const NavDropdown = memo(function NavDropdown({ item }: Readonly<{ item: NavItem }>) {
  ...
  return (
    <div className="relative" onTouchStart={() => { setOpen(prev => !prev) }}>
      <NavLink
        to={item.to}
        onMouseEnter={enter}
        onMouseLeave={leave}
      ...
      {open && (
        <div
          className="absolute top-full start-0 z-50 pt-1"
          onMouseEnter={enter}
          onMouseLeave={leave}
        >
      ...
      <div className="max-w-[1280px] mx-auto px-5 h-16 flex items-center justify-between gap-3">
        ...
        <div className="hidden xl:flex items-center gap-2 shrink-0">
          ...
          <a href={`https://wa.me/${company.whatsapp}`} target="_blank" rel="noopener noreferrer"
            aria-label="WhatsApp"
            className="group relative overflow-hidden flex items-center justify-center bg-whatsapp text-white w-9 h-9 rounded-full shrink-0">
            <span className="absolute inset-0 rounded-full bg-lime scale-0 group-hover:scale-100 transition-transform duration-500 ease-in-out" />
            <MessageCircle size={17} className="relative z-10 transition-colors duration-300 group-hover:text-forest" />
          </a>
          <Link to="/contact-us/"
      ...
      {open && (
        <div className="lg:hidden bg-white border-t border-border px-5 py-4 flex flex-col gap-1 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex gap-2 mt-3">
            ...
            <a href={`https://wa.me/${company.whatsapp}`} target="_blank" rel="noopener noreferrer"
              className="group relative overflow-hidden flex-1 flex items-center justify-center gap-1.5 bg-whatsapp text-white text-sm font-semibold px-4 py-2.5 rounded-full">
              <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[220%] aspect-square rounded-full bg-lime scale-0 group-hover:scale-100 transition-transform duration-500 ease-in-out" />
              <MessageCircle size={14} className="relative z-10 transition-colors duration-300 group-hover:text-forest" />
              <span className="relative z-10 transition-colors duration-300 group-hover:text-forest">{t('header.whatsapp')}</span>
            </a>
